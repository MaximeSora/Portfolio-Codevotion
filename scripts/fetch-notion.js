...
async function main() {
  if (!process.env.NOTION_TOKEN || !DATABASE_ID) {
    const missing = [
      !process.env.NOTION_TOKEN ? 'NOTION_TOKEN' : null,
      !DATABASE_ID ? 'NOTION_DATABASE_ID' : null,
    ].filter(Boolean).join(', ');
    console.warn(`⚠️ Variables manquantes: ${missing}`);

    if (!existsSync(PROJECTS_JSON)) {
      mkdirSync(resolve(ROOT, 'src/data'), { recursive: true });
      writeFileSync(PROJECTS_JSON, '[]\n', 'utf-8');
      console.warn('⚠️ Fallback créé: src/data/projects.json (tableau vide).');
    } else {
      console.warn('ℹ️ src/data/projects.json existant conservé.');
    }
    return;
  }
